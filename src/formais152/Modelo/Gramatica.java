...
public class Gramatica {
	...
	private boolean corpoDaProducaoEhValido(String corpo) {
		...
		if (corpo.equals("&") && simbolosTerminais.contains(corpo)) {
			throw new ProducaoMalFormadaException("Já existe uma produção que deriva epsilon");
		}

		return (corpo.length() == 1 && !Character.isUpperCase(corpo.charAt(0))) || (corpo.length() == 2
				&& !Character.isUpperCase(corpo.charAt(0)) && Character.isUpperCase(corpo.charAt(1)));
	...
	public boolean adicionaProducao(String cabecaProducao, String corpoProducao) {
		...
		throw new ProducaoMalFormadaException("Produção " + cabecaProducao + " -> " + corpoProducao + " inválida");
		...
		for (String nt : naoTerminais) {
			...
			for (String t : transicoes) {
				if (t.length() == 1 && !t.equals("&")) {
					...
					try {
						automato.addTransicao(nt, t.substring(0, 1), t.substring(1, 2));
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
